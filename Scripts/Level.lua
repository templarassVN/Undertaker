...
function Level:init(def)
    ...
    self._Princess = def.Princess:clone()
    ...
    self._Room:set_Tile_stt(self._Princess._grid_x,self._Princess._grid_y,false)
...
function Level:update(dt)
    ...
    self._Player:update(dt,self._Room,self._Objects,self._NPCs,self._Traps,self._Key, self._Chest, self._Princess)
    
    for i,object in pairs(self._Objects) do
        object:update(dt)
    end
    
    if (self._Player._iswining == true) then
        self._isfinish = true
    end

    if(self._Room._moveremain <= 0) then
        gSounds['dead']:play()
